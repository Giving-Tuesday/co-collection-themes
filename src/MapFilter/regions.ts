/**
 * Mapping of human-readable region/country strings to ISO alpha-3 country codes.
 * Broad regions expand to arrays of codes.
 */
...
export const COUNTRY_TO_ISO3: Record<string, string[]> = {
  ...
  'Sub-Saharan Africa': [
    'AGO', 'BEN', 'BWA', 'BFA', 'BDI', 'CMR', 'CAF', 'TCD', 'COM', 'COD',
    'COG', 'CIV', 'DJI', 'GNQ', 'ERI', 'ETH', 'GAB', 'GMB', 'GHA', 'GIN',
    'GNB', 'KEN', 'LSO', 'LBR', 'MDG', 'MWI', 'MLI', 'MRT', 'MUS', 'MOZ',
    'NAM', 'NER', 'NGA', 'RWA', 'STP', 'SEN', 'SLE', 'SOM', 'ZAF', 'SSD',
    'SDN', 'SWZ', 'TZA', 'TGO', 'UGA', 'ZMB', 'ZWE',
  ],
  ...
  Asia: [
    ...
    'GEO', 'IND', 'IDN', 'IRN', 'IRQ', 'ISR', 'JPN', 'JOR', 'KAZ', 'PRK',
    'KOR', 'KWT', 'KGZ', 'LAO', 'LBN', 'MYS', 'MDV', 'MNG', 'MMR', 'NPL',
    'OMN', 'PAK', 'PHL', 'QAT', 'SAU', 'SGP', 'LKA', 'SYR', 'TWN', 'TJK',
  ...
  'Global': [], // special case handled in component
};
...
export function resolveIsoCodes(options: string[]): Set<string> {
  ...
  for (const option of options) {
    const mapped = COUNTRY_TO_ISO3[option];
    if (mapped) {
...
/**
 * Given a set of ISO3 codes, find which original option strings they belong to.
 * Returns the first matching option string for a given code.
 */
